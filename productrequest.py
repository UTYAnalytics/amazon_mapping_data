...
class KeepaException(Exception):
    """Custom exception class for Keepa API errors."""
    ...
    def __str__(self):
        base_message = super(KeepaException, self).__str__()
        additional_info = []
        if self.status_code is not None:
            additional_info.append(f"Status Code: {self.status_code}")
        if self.response_data:
            additional_info.append(f"Response Data: {self.response_data}")
        if additional_info:
            return f"{base_message} ({', '.join(additional_info)})"
        return base_message
...
class KeepaAPI:
    ...
    def products(
        ...
        update=None,
        history=True,
        offers=100,  # not offer/ check IP brand first
        rating=True,
        domain=None,
        buybox=1,
    ):
    ...
    def sellers(self, seller_ids, storefront=False, update=None, domain=None):
        ...
        if storefront and len(seller_ids) > 1:
            raise KeepaException(
                "Seller id batch requests are not allowed when requesting the storefront."
            )
        query = {
            "key": self.key,
            "domain": domain or self.domain,
    ...
    def request(self, path, query):
        ...
        url = urllib.parse.urlunparse(
            ("https", "api.keepa.com", path, "", urllib.parse.urlencode(query), "")
        )
        while True:
            counter = 3
            while True:
            ...
            if self.response.status_code == 200:
                ...
                if self.response.status_code == 429:
                    ...
                    logger.info(
                        "Tokens left {}. Sleep {} sec.".format(
                            data["tokensLeft"], delay
                        )
                    )
                    time.sleep(delay)
                else:
...
# interpolate(a.products(['1416580514']).get("products")[0]["csv"])
